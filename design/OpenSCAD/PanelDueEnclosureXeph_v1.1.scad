...
// Based further on the modification by dc42 - you rock!
// Updated 2018-06-02 by Xepherys to support proper lid functionality in some cases and offer brass
...
// Xepherys (6/2/18) Note: The Lid option prints a lid, the Screw option provides Screw holes for a lid.  Either properly resize the box.
// Do you want a lid?
Lid = 0;						    // [0:No,1:Yes]
...
// Brass Insert Options (Xepherys 6/2/18)
// Each adds a widened top for the Screwn hole for easy insertion of brass inserts.
//  Cover - the four cover Screws.  Only applicable if 'Screw' bool == true
//  Use 0 or 1 rather than true or false as they are used for int calculations as well.
BrassForCover = 0;   // [0:No,1:Yes]
BrassForScreen = 0;  // [0:No,1:Yes]
...
Mclearance =  (Lid || Screw) ? 5 : 1;		// clearance between board edge and inside of box
MBez = 1;							// This is the thickness covering the non-visible part of the LCD display (MBase thickness - recess)
...
Lidmnthole = 2 + BrassForCover;	// Hole to take self-tapping Screw or not if using brass inserts.
Lidmntclearhole = 2.2 + BrassForCover;		// hole clearence for lid Screws
...
overlap = 0.1;
$fn = 30;  // 50 is good for printing, reduce to render faster while experimenting with parameters
	...
	difference()
	{
        ...
        if (BrassForScreen)
        {
            for (Mnt = LCDmounts)
            {
                ...
                rotate([0,0,45])
                translate([0, 0, LCDheight - BrassInsertHeight]) cylinder(r = (BrassInsertWidth *5), h = BrassInsertHeight);
             }
        }
	}
...
module Lidmnt()  // This creates the mounts for the Lid
{
	...
	rotate([0, 0, 45])
	Boss(Lidmnthole, LidbossDia, LidbossD2, MHt, LidsupW, BrassForCover);
}
...
module PCBmnt()
{
	...
	for (Mnt = PCBmounts) {
		translate([Mnt[0],Mnt[1],MBez])
			rotate([0,0,45])
				Boss(PCBmnthole,PCBbossDia,PCBbossD2,PCBheight+LCDheight,PCBsupW, BrassForScreen);
	}
}
...
module LCDmnt()
{
		...
		for (Mnt = LCDmounts) {
			translate([Mnt[0],Mnt[1],MBez])
				rotate([0,0,45])
					Boss(LCDmnthole,LCDbossDia,LCDbossD2,LCDheight,LCDsupW, BrassForScreen);
		}
}
...
module Boss(HoleD, Dia1, Dia2, Ht, MWall, brass=0)
{
	difference()
	{
		union()
		{
			cylinder (r = (Dia1 + (brass * 3.5)) / 2, h = Ht);
			Xsupport(Dia1, Dia2 + (brass / 2), Ht, MWall);
			CirFillet(Dia1, MWall);
		}
        
        // This creates the screw hole in the boss structure.
		translate([0, 0, -0.1]) cylinder(r = HoleD / 2, h = Ht + 0.2);
        
        // This adds the spacing for a brass insert at the top of the boss.
        if (brass)
            translate([0, 0, Ht - BrassInsertHeight]) cylinder(r = (BrassInsertWidth / 2), h = BrassInsertHeight);
	}
}
...
module Box(){
	// RoundBox(MLen+2*MWall,MWid+2*MWall,MHt+MBase,MRad,MWall,MBase);
	 if(Screw){
       Lidmnt();  // Xepherys (6/2/18) Changed order of if-statements for 'Screw' and 'Lid' bools so that the box can be made with Screw mounts without making the lid.
		if (Lid){ 
			//Lidmnt();
			if (Right){
				difference(){ 
